...
$if(title)$
\title{$title$$if(subtitle)$: $subtitle$$endif$ $if(anonymous)$$else$$endif$ }
$endif$ 



$if(anonymous)$$else$\author{$for(author)$\Large $author.name$$if(thanks)$\thanks{$thanks$} $endif$ \vspace{0.05in} \newline\normalsize\emph{$author.affiliation$}  $sep$ \and $endfor$}$endif$
...
\begin{document}
	...
	\begin{abstract}
		...
		\vskip 8.5pt \noindent \emph{JEL Codes}: $jelcodes$ \par
