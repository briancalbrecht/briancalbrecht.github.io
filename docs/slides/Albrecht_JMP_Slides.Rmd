...
date: '`r format(Sys.time(), "%B %d, %Y")`<br><br>Paper `r icon::fa("link")`  [bit.ly/Albrecht-JMP](http://bit.ly/Albrecht-JMP) <br>Slides `r icon::fa("link")`  [bit.ly/Albrecht-JMP-Slides](http://bit.ly/Albrecht-JMP-Slides) <br>`r icon::fa("twitter")` [briancalbrecht](https://twitter.com/briancalbrecht)'
output:
...
- Price transfer: $p : B \times S \to \mathbb{R}$
  - Transfer from buyer with $b$ to seller with $s$
  
- Total buyer utility: $v(b,s) - p(b,s) - \frac{1}{4}b$

- Total seller utility: $p(b,s) - \frac{1}{4}s$
...
- Agents make their decisions based on some price conjectures: $\tilde{p}(b,s)$

- Buyer's problem

$$\max_{b'} \left\{\max_{s'} v(b',s') - \tilde{p}^i(b',s') - \frac{1}{4}b'\right\}$$


- Seller's problem 

$$\max_{s'} \left\{\max_{b'}  \tilde{p}^j(b',s') - \frac{1}{4}s'\right\}$$
...
2. Each seller $j$ chooses $(b,s)$ to maximize utility, given price $\tilde{p}^j(b,s)$

3. Rational conjectures: conjectures are not contradicted by the data
  - If positive mass of agents choose $(b,s)$, conjectures agree with the observed price: $\tilde{p}^i (b,s) = \tilde{p}^j (b,s)= p(b,s)$
  
  - Otherwise conjectures are not pinned down 

4. Prices clear matching markets
  - Mass of buyers choosing $(b,s)$ = mass of sellers
...
- If a match $(b,s)$ is chosen, $\tilde{p}^i (b,s) = \tilde{p}^j (b,s)= p(b,s)$
...
- Coordination failure: $b=s=0$
  ...
  - Supposed ralized prices: $p(0,0)=0$
  - **Contradicting** conjectures: $\tilde{p}^i (1,1) = 1$ and $\tilde{p}^j(1,1) = 0$
...
- Reason for multiplicity: Off-path conjectures are a free parameter
  - Theorist can pick $\tilde{p}^j(b,s)$ sufficiently low to discourage all other actions
...
- No mistakes/trembles in matching market 

- Symmetric trembling hand: each attribute must be chosen with positive probability $\epsilon^i>0$ by each buyer and seller

- Assume in the aggregate each attribute must be chosen by a positive density of players

--

- Perfect equilibrium: limit of some sequence of $\epsilon = (\epsilon^i,\epsilon^j)$
...
- If $p(1,1)$ is high $(> \frac{1}{4})$, sellers want to choose $s=1$ as much as possible: $1 - \epsilon^i$
  - Drives price down

--

- If $p(1,1)$ is low $(< \frac{3}{4})$, buyers want to choose $b=1$ as much as possible: $1 - \epsilon^j$
  - Drives price up

--

- Investment equilibrium:  $p(1,1) \in \left[\frac{1}{4}, \frac{3}{4}\right]$


- As perturbations go to zero, $(1,1)$ is the still unique equilibrium strategy profile
...
Buyers optimize their investment: for all $(b,t) \in \text{supp } \nu$


$$v_b^*(\tilde{p}) - c(b,t) \geq v^*_{b'}(p) - c(b',t) ~~~\forall b' \in B$$
...
- For all sellers $\epsilon_S = (\epsilon(b,s))_{s \in S},$ satisfying  $\epsilon(b,s)>0$ for all $s \in S$ and 
$$\sum_S \epsilon(b,s) \leq 1$$
...
- A pair $(\nu(\epsilon), p)$ is an $\epsilon$-investment equilibrium for $E$ if $\nu$ is $\epsilon$-feasible, $p$ is a competitive price for $\mu$, and for all $(a,t)$ such that $\nu_{A}(\epsilon) > \epsilon$,
...
$$Q(\epsilon)= \sum_a \sum_{\nu} \left[v^*_a(p) - c(a,t)\right]\epsilon(a)$$
...
$$\underbrace{\left(\int  \left[\max_b v(b,s) - \tilde{p}^i (b,s) - c(b,i)\right] di  + \int\left[\max_s  \tilde{p}^j (b,s)  - c(s,j)\right] dj\right) \left(1 - \sum_a \epsilon(a) \right)}_{\text{Optimized Choice}}$$
...
- In an competitive matching model with investment, every perfect equilibrium is efficient
