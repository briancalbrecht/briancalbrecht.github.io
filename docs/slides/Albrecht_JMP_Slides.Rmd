...
- Off-path conjectures are a free parameter
  - Lucas/Sargent: beware of theorists bearing free parameters 

- With no condition, two investment equilibria:
  - Efficient: $b=s=1$
  - Coordination failure: $b=s=0$

- Basically no predictive power from a Nash-style equilibrium

- The perfect equilibrium refinement generates predictions

---
Perturbed Strategies
====================================

- Uniform trembling hand: each attribute must be chosen with positive probability density $\epsilon>0$ by each buyer and seller
...
- For any perturbation, equilibrium requires
...
- For each $b \in  \text{supp } \mu$ and each $(s) \in \text{supp } x,$

$$v(s) - p(s)= v_b^*(p) \equiv \max_{s' \in S^0} v(b,s') - p(b,s')$$
--

- For each $s \in  \text{supp } \mu$ and each $(s) \in \text{supp } x,$, $b \in B$ implies 
