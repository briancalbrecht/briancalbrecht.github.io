...
date: '`r format(Sys.time(), "%B %d, %Y")`<br><br> `r icon::fa("link")` [bit.ly/Albrecht-JMP-Slides](https://bit.ly/Albrecht-JMP-Slides) <br>`r icon::fa("twitter")` [briancalbrecht](https://twitter.com/briancalbrecht)'
output:
...
- Formally, inefficient equilibria are not robust to errors like a trembling-hand refinement


- We should predict efficient market outcomes
...
<font color=#125972><b>Walrasian equilibrium refinements:</b></font> Gale (1992, 1996), Dubey & Geanakoplos (2002), Dubey, Geanakoplos, & Shubik (2004), Zame (2007), Scheuer & Smetters (2018)
...
- Surplus $v(b,s) = bs$ is split through transfer to seller through price

- Let $p(s) \equiv \max_b p(b,s)$ so that price is only a function of $s$
  - Without loss since the seller does not care which $b$ pays her 
  
- Total buyer utility: $v(b,s) - p(s) - \frac{1}{4}b$

- Total seller utility: $p(s) - \frac{1}{4}s$
...
1. Prices clear matching market
...
- Efficient equilibrium: $b=s=1$
  ...
  - Realized prices: $p(1) \in \left[\frac{1}{4}, \frac{3}{4}\right]$
  - Non-contradicting conjectures: $\tilde{p}^i(0) \geq 0$ and $\tilde{p}^j(s) \leq 1/2$
  
--

- Coordination failure: $b=s=0$
  - Realized prices: $p(0)=0$
  - Contradicting conjectures: $\tilde{p}^i(1) \geq 3/4$ and $\tilde{p}^j(1) \leq 1/4$

--

- What can we predict?
  - Everyone can invest (max surplus) or not (min surplus)

- Little predictive power from a Nash-style equilibrium

???
SLOW SLOW SLOW

---
background-image:url(https://briancalbrecht.github.io/docs/slides/images/jmp/tikz_D.png)
background-size:contain

???

This is display of surplus equilibrium surplus

Ignoring any mixed strategies or mixed prices

---

- With continuum of actions, $B=S=[0,1]$, the set of equilibria explodes

- Suppose $b=1$ for all buyers. Which $s$ can be sustained as an equilibrium? 
  -  Any $s \geq 1/3$

- Construction: $p(s^*)= \frac{1}{4}s^*$ and $\tilde{p}^j(s)=0$ for all $s\neq s^*$
  - Sellers are discouraged from deviating by pessimistic beliefs
  - Buyers: $s^* - 1/4 s^* - 1/4 \geq0$

--

- Reason: Off-path conjectures are a free parameter
  - Theorist can pick $\tilde{p}^j(s)$ sufficiently low to discourage all other actions
...
- Back to two actions example: $B=S=\{0,1\}$

- Symmetric trembling hand: each attribute must be chosen with positive probability density $\epsilon^i>0$ by each buyer and seller

- Assume in the aggregate each attribute must be chosen by a positive density of players
  
- Perfect equilibrium: limit of some sequence of $\epsilon$
...
- With trembling hand, each $b$ and $s$ are played $\Rightarrow$ $\tilde{p}^i(s) = \tilde{p}^j(s) = p(s)$



- If $p(1) > \frac{1}{4}$, sellers want to choose $s=1$ as much as possible: $1 - \epsilon^i$
  - Drives price down

- If $p(1) < \frac{3}{4}$, buyers want to choose $b=1$ as much as possible: $1 - \epsilon^j$
  - Drives price up

- Investment equilibrium:  $$p(1) \in \left[\frac{1}{4}, \frac{3}{4}\right]$$
...
- I assume "naive" perfect competition in the continuum through price-taking
...
- Bounded continuous value function: $v : B^0 \times S^0 \to \mathbb{R}$
  ...
  - When $v$ is ont submodular, coordination problems are possible*
...
<font color=#125972><b>Definition:</b></font> The pair $(x,p)$ is an (ex post) **Walrasian equilibrium** for $\mu$ if $x$ is feasible for $\mu$, $p(0,s)= p(b,0) \equiv 0$,
--

- For each $b \in  \text{supp } \mu$ and each $(b,s) \in \text{supp } x,$

$$v(s) - p(b,s)= v_b^*(p) \equiv \max_{s' \in S^0} v(b,s') - p(b,s')$$
--

- For each $s \in  \text{supp } \mu$ and each $(b,s) \in \text{supp } x,$, $b \in B$ implies 

$$p(b,s) = v_s^*(p) \equiv \max \left\{p(b,s), v(0,s)\right\},$$
...
- Investment equilibria need not be unconditionally efficient

- Consider again the example $B=S=\{0,1\}$

- Surplus: $v(s) = b s$

- For all buyers, $b=0$, for all sellers $s=0$, and $p(b,s)=0$ is an investment equilibrium
...
- Suppose not matching is generates no value generation: $v(0,s)=v(b,0)=0$
...
- Why would sellers conjecture $p(1) = 0$?
...
- Approaches on previous slide would directly extend Theorem 1 to finite players

- In general, trembling hand does not offer a refinement with finite players
