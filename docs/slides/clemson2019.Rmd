...
date: '`r format(Sys.time(), "%B %d, %Y")`<br><br> Paper `r icon::fa("link")`  [bit.ly/bca-evolution](http://bit.ly/bca-evolution) <br>`r icon::fa("twitter")` [briancalbrecht](https://twitter.com/briancalbrecht)'
output:
...
First, government failure theory:
...
Second, Chicago school of political economy
...
- In standard model, tariffs are inefficient: $B < C$
...
- Once $C$ crosses some threshold, consumers will enter

- Threshold depends on $O_C$ and the time-series properties of $C$
...
- If $N>0$, current policy is **inefficient**
